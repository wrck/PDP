 ...
 *
 * <p>这与 {@code com.pdp.shared.page.CursorCodec}（HMAC 签名 keyset cursor）的契约一致——
 * 游标对客户端不透明——但避免引入签名密钥配置；后续可替换为签名实现而不改变端口契约。
 *
 * <p>当 {@code lastId} 为 {@code null}（首页）时返回 {@code null}。
 */
public final class WorkspaceCursorCodec {

    private static final Base64.Encoder ENCODER = Base64.getUrlEncoder().withoutPadding();
    private static final Base64.Decoder DECODER = Base64.getUrlDecoder();

    private WorkspaceCursorCodec() {
    }
