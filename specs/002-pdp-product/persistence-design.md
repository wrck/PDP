...
本设计落实 FR-151～FR-162，统一 PostgreSQL 18、MySQL 8.4、MyBatis-Plus、动态数据源、连接池、游标分页、并发控制、类型映射、索引投影、搜索和事务事件语义。

核心边界：

- 每个 PDP 部署只有一个业务写入主库。
- 动态数据源用于受控访问业务主库、可选只读副本和迁移源/目标，不用于把数据库切换伪装成运行时热切换。
...
| `migrationSource` | 历史 MySQL 或数据库切换源 | 否 | 迁移计划批准后加载，结束后卸载并撤权 |
| `migrationTarget` | 数据库切换目标的预装载和核对 | 仅迁移执行器 | 切换期临时存在，开放业务写入前不得作为 `pdpPrimary` |
...
| `initializationFailTimeout` | `pdpPrimary` 必须启动失败即退出；临时迁移源由迁移作业报告失败 |
| `leakDetectionThreshold` | 非生产或故障诊断时启用，生产默认关闭 |

所有应用副本与后台执行器的连接池上限之和不得超过数据库可用连接的 70%，其余连接留给高可用、迁移、维护和应急。必须监控 active、idle、pending、timeout、acquire duration、usage duration 和连接创建失败。使用 JDBC4 `isValid()`，不配置数据库专用探活 SQL；操作系统和驱动必须启用 TCP keepalive。
...
游标使用 Base64URL 编码和 HMAC 签名，不包含数据库类型、表名或原始 SQL。过滤条件、排序方式或权限范围改变时旧游标必须失效。所有可分页查询必须定义稳定排序，最终排序键为 `id`；PostgreSQL/MySQL 适配器必须实现相同的 NULL 排序和复合 keyset 谓词。默认不执行总数查询。
...
| 逻辑类型 | Java | PostgreSQL | MySQL | 规则 |
|---|---|---|---|---|
| UUIDv7 | `UUID` | `uuid` | `BINARY(16)` | 使用 RFC 4122 网络字节序，不调用数据库生成函数 |
| `Instant` | `Instant` | `timestamptz(6)` | `datetime(6)` | JDBC 会话 UTC，微秒精度，读取后仍为 Instant |
| `JsonDocument` | Jackson `JsonNode` | `jsonb` | `JSON` | 使用统一 Jackson 配置；计算哈希时采用规范化 JSON |
| 稳定枚举键 | `String`/值对象 | `varchar` | `varchar` | 保存稳定键，禁止 ordinal |
| 金额/权重 | `BigDecimal` | `numeric(p,s)` | `decimal(p,s)` | 精度、舍入和溢出由字段定义控制 |
| `ObjectRef`/`ActorRef` | 值对象 | 分列或规范化 JSON | 分列或规范化 JSON | 核心引用优先分列，不以数据库对象类型保存 |
...
P1 使用平台统一分析器生成 `SearchDocument` 和 `SearchTermProjection`，统一 Unicode 规范化、大小写折叠、停用词、字段权重和词项版本。PostgreSQL FTS、MySQL FULLTEXT 或专用索引只能加速候选检索：
...
历史迁移和数据库切换使用独立 `SqlSessionFactory`、Mapper 扫描包、事务管理器和 HikariCP 池。业务 Mapper 不能注入迁移数据源，迁移 Mapper 不能使用业务管理权限。
...
- 数据库编码 UTF-8，时区 UTC，使用批准且可在目标环境重建的 ICU collation。
- P1 不强制安装扩展；所有可选扩展进入能力矩阵。
- 连接开启 TCP keepalive；大表索引使用受控在线构建方案。
...
- PostgreSQL/MySQL 分别验证空库初始化、逐版本升级和失败恢复。
- 游标分页覆盖正反向、NULL、并发插入、权限变化和游标篡改。
- 自定义 Mapper 更新覆盖 revision 成功、冲突、越权与批量部分失败。
- TypeHandler 对 UUID 字节序、JSON、Instant、枚举、NULL 执行双数据库往返测试。
- 动态数据源覆盖严格路由、事务内切换拒绝、上下文清理和连接池耗尽。
- 搜索分析器和投影结果在两种数据库上一致；原生搜索优化关闭时仍正确。
