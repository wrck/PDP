...
**完成原则**：API 契约、状态机和领域失败测试先于对应实现；依赖真实页面、数据库或外部服务的 E2E/集成测试在实现可运行后执行。持久化故事最终必须具备公共变更集、仓储端口、MySQL Mapper/适配器和 MySQL 契约测试；公共契约位于 `public-persistence`，MySQL 专有实现位于 `persistence-mysql`。单个实现任务通过受影响模块的最小验证即可完成，权限、审计、可观测、完整回归和闭环证据集中在用户故事收口任务。
**独立实施单元**：多任务并行实施时，从 `implementation-units/README.md` 领取一个独占文件范围；原 Task 状态仍只在本文件维护，并由收敛点根据已合入提交统一更新，实施单元不构成第二套任务清单。
...
- [X] T096 执行基础阶段门禁并记录平台工作流、MySQL 持久化边界、权限和迁移隔离证据，文件：`specs/002-pdp-product/evidence/foundation-validation.md`
...
- [X] T127 [US2] 创建网络设备割接示例包并记录状态机、日志指标追踪、独立审核、影响预览和发布闭环证据，文件：`tests/fixtures/domain-package/network-cutover-package.json`、`specs/002-pdp-product/evidence/us2-domain-package.md`
...
以下定义用于用户故事收口，不作为故事内每个实现任务的重复门禁。实现期间沿“契约/领域模型 → 持久化 → 应用服务 → API/页面 → E2E/证据”的可运行关键路径推进；标记为 `[P]` 但依赖尚未就绪的测试任务可延后到对应实现之后。
