...
## 决策 4：持久化核心保持数据库独立，P1 只认证 MySQL 8.4 LTS

**决定**：P1 任一时刻使用一个 MySQL 8.4 LTS 受管部署作为业务事实真源，但可登记多个 MySQL 部署，并通过统一数据库切换接口、状态机、迁移运行和高风险治理完成 `MYSQL→MYSQL` 写入主权切换。核心字段使用规范化关系模型；可扩展对象使用逻辑 JSON、通用实例、关系和类型化索引投影表。领域层和应用层只依赖持久化端口，MySQL 差异集中在 `persistence-mysql` 的适配器、方言、映射、查询实现和 Liquibase 专用变更集中。PostgreSQL 进入 P2 时仅新增适配器、认证矩阵和获批组合，复用 P1 切换执行器。

**理由**：P1 需要同时完成平台内核、Flowable、标准实施领域包和现有 MySQL 历史数据迁移。双数据库同步实现会扩大 DDL、SQL、索引、性能、恢复和迁移测试面。MySQL 8.4 LTS 能支撑核心 OLTP，并与现有数据和运维能力最接近。通过公共持久化端口、逻辑类型、能力画像、适配器注册和架构测试保留数据库独立性，可以先降低首期交付风险，同时避免在 P2 启动时重新破坏领域边界，再以认证矩阵验证 PostgreSQL，而不是在 P1 维护两套未经充分验证的生产实现。

运行中直接修改连接字符串仍不能安全切换目标，因为 schema 版本、数据内容和写入主权无法自动一致。P1 同时支持历史 MySQL 旧系统到 PDP 的 `CUTOVER`，以及两个 PDP MySQL 受管部署之间的 `DATABASE_SWITCH`；两者使用不同业务类型，但共享全量、增量、核对、冻结、单写主权和回退基础设施。任一时刻只允许一个写入主部署；P1 不支持跨数据库分片、在线双主或无迁移热切换。
...
- P1 同时实现 PostgreSQL 与 MySQL：覆盖面更大，但会显著增加首期 DDL、查询、性能、恢复和迁移风险，推迟到 P2。
- 仅支持 MySQL 且业务代码直接使用专有能力：短期最快，但形成硬绑定，不采用。
- 抽象到所有关系数据库：范围过大且无法形成可信认证矩阵；后续数据库必须通过同一持久化端口和发布门禁加入。
...
**决定**：P1 以 MyBatis-Plus 3.5.17 Spring Boot 4 Starter 作为首个验证基线和唯一主持久化框架，正式实现时由项目 BOM 锁定经 MySQL 8.4 验证的补丁版本。简单单表操作使用受约束的 `BaseMapper` 能力，复杂查询、权限过滤、批量装载、搜索读模型和报表使用显式 Mapper SQL；业务模块只依赖自定义仓储或查询端口，不依赖 MyBatis-Plus 类型。Hibernate/JPA 不进入 P1 业务持久化栈。

**理由**：PDP 的主要持久化难点不是实体 CRUD，而是工作空间与对象级权限、动态字段投影、跨对象查询、审批待办、搜索、报表和批量迁移。MyBatis/MyBatis-Plus 能让 SQL、索引命中和锁范围保持显式；MyBatis-Plus 还提供分页、乐观锁和阻止全表更新删除等基础插件。其 Spring Boot 4 Starter 已由官方提供。
...
- 公共 SQL 使用可移植子集；MySQL 专用语句位于独立 Mapper 适配器，不在业务代码中判断数据库类型。
- 乐观并发统一使用 `revision`；高风险更新同时校验主键、工作空间、当前 revision 和权限范围。
- 禁止使用 `${}` 接收用户输入；动态表名、排序字段和列选择必须来自平台白名单。
- Mapper SQL 必须在 MySQL 8.4 Testcontainers 上执行契约、权限和性能测试。
...
**决定**：首个验证基线采用 `dynamic-datasource-spring-boot4-starter 4.5.0`，设置 `primary=pdpPrimary`、`strict=true`。在线路由键为业务主库 `pdpPrimary` 和可选只读副本 `pdpRead`。迁移控制面注册 `migrationSource` 和 `migrationTarget`，但二者分别使用专用 `SqlSessionFactory`、Mapper 扫描包和本地事务管理器。平台工作流使用独立 `workflowEngine` 数据源、schema/表前缀、账号、本地事务管理器和 HikariCP 池，不加入普通业务动态路由，也不与 `pdpPrimary` 使用同一连接池。所有池版本由 Spring Boot BOM 管理。

**理由**：历史 MySQL 迁移期间需要同时访问旧系统源库与 PDP MySQL 目标库；部分后续报表可能需要只读副本。动态数据源能统一路由和连接生命周期，但若允许业务代码自由使用 `@DS`，会把数据主权、事务和读一致性变成隐式行为。因此路由只存在于基础设施层，事务入口确定后不可切换。HikariCP 配置简单、监控成熟，适合为不同用途建立独立连接预算。

P1 不使用 XA、JTA 或 Seata。跨数据库迁移使用只读源事务、独立目标批次事务、幂等批次键、检查点和业务核对。动态加载 `migrationSource`/`migrationTarget` 必须由迁移计划控制并审计；普通业务接口不能动态增加或删除数据源，业务 Mapper 不能注入迁移会话工厂。
...
- `workflowEngine` 使用与当前部署相同的认证数据库产品和 UTC/隔离级别语义，但独立账号、schema、
  连接池、事务管理器和指标。
- 监控 active、idle、pending、timeout、获取耗时、使用耗时和连接创建失败。

**备选方案**：

- 所有数据源共用一个 `SqlSessionFactory`：配置较少，但无法可靠隔离业务 Mapper、迁移 Mapper 和本地事务边界，不采用。
...
- UUIDv7、Instant、JSON、稳定枚举和公共值对象使用统一 TypeHandler 与 MySQL 往返测试。
- MyBatis 二级缓存和懒加载关闭，`localCacheScope=STATEMENT`，未知列映射失败。
- MySQL JDBC 会话统一 UTC、`READ_COMMITTED`；业务唯一性使用应用规范值或摘要，不依赖默认 collation。

**理由**：MyBatis-Plus 的默认分页和乐观锁只覆盖部分内置方法，不能自动保证平台游标和所有自定义 SQL 的一致性。MySQL 在 UUID、JSON、日期时间、NULL、排序和默认隔离级别上仍存在隐式语义，必须在持久化契约中固定，而不是依靠开发人员逐查询记忆。

详细契约见 [persistence-design.md](persistence-design.md)。

## 决策 5：搜索与报表先使用数据库读模型

**决定**：P1 使用平台统一分析器生成 `SearchDocument` 和 `SearchTermProjection`，统一词项、字段权重和稳定排序；搜索索引更新由事务事件驱动。MySQL FULLTEXT 或其他数据库索引只作为候选加速，最终权限过滤、结构化过滤、匹配集合和排序由平台搜索契约决定。只有在 MySQL 8.4 真实压测无法满足 SC-018 时，才引入 OpenSearch 等独立搜索集群。
...
## 决策 7：Flowable 作为平台工作流基础能力，核心业务事实仍由领域实现

**决定**：P1 采用 Flowable 8.0.x Process Engine 作为 BPMN 2.0.2 工作流编排引擎，由项目 BOM 锁定
经过 Java 21、Spring Boot 4.1 和 MySQL 8.4 验证的补丁版本。只引入
`flowable-spring-boot-starter-process`，不引入全引擎 starter、Flowable REST、IDM、CMMN、DMN
或 JPA/Hibernate 集成。项目、任务和交付件状态机仍由领域元数据定义；审批继续使用独立聚合保存
定义、实例、节点、决定、意见和业务回写。Flowable 只负责路由、并行/汇聚、等待、定时、超时、
服务任务和补偿编排。

PDP 增加平台级 `workflow` 公共模块，提供定义、运行、人工任务和管理四类稳定端口。审批、项目、
交付件、治理对象和领域包运行时只传入稳定业务引用、流程键、流程版本、幂等键和最小变量，不依赖
Flowable API、表结构或异常类型。流程定义可与平台能力或领域包版本关联，
已运行实例固定启动版本；流程迁移必须预览、分批、暂停、恢复并保留证据。Flowable 使用独立 schema
或表前缀、账号、HikariCP 池和本地事务管理器；生产关闭自动建表/升级。业务事务与引擎事务不使用
XA，而是通过 Spring Modulith outbox 和幂等事件桥接启动、推进及回写流程。

**理由**：PDP 的审批、超时、退回、加签、并行会签和长时等待需要成熟 BPMN 编排，但项目生命周期、
审批结论、对象版本、权限和审计不能被流程引擎私有状态替代。受限适配边界兼顾 Flowable 的流程能力
与平台统一查询、领域包组合、数据库独立边界、失败恢复和未来替换能力。Flowable 官方文档确认支持
Spring Boot 4.x，提供仅启动 Process Engine 的 starter；Flowable 8 要求 JDK 17 以上，符合 Java 21
基线。避免 JPA 集成也防止 Hibernate 作为第二套业务持久化框架进入运行时。

**备选方案**：

- Flowable 作为全部业务对象的全局状态运行时：建模统一，但会把领域事实、授权和查询口径锁入流程实例。
- 不使用流程引擎，仅自研审批路由：短期依赖少，但定时、并行、补偿、实例迁移和可视化诊断成本高。
- 每个模块自建审批：会造成待办、委托、审计、权限和升级规则不一致。
- 引入全 Flowable starter 或 REST 应用：快速获得更多引擎和 API，但扩大攻击面、依赖和数据模型，
  且与 PDP 自有身份、授权和契约边界重复。

**来源**：

- [OMG BPMN 2.0.2](https://www.omg.org/spec/BPMN/2.0.2/)
- [Flowable Spring Boot 集成与 starter 列表](https://www.flowable.com/open-source/docs/bpmn/ch05a-Spring-Boot/)
- [Flowable Open Source 运行要求与许可](https://www.flowable.com/open-source/docs/oss-introduction/)
- [Flowable 8.0.0 API 文档](https://www.flowable.com/open-source/docs/all-javadocs/)
...
- 单元测试覆盖规则、计算和状态转换。
- 模块测试验证模块边界与模块内集成。
- Testcontainers 对 MySQL 8.4 执行完整持久化契约、schema 升级、并发和模块集成测试，并独立验证 MySQL 历史迁移源、Redis 和对象存储适配。
...
**决定**：迁移分为盘点、映射、全量快照、持续增量、转换装载、业务核对、冻结切换和只读保留。MySQL 原始数据先进入隔离暂存层，再由版本化映射转换为 PDP 核心命令或受控批量装载；P1 目标固定为 PDP MySQL 8.4。数据库搬运工具不得直接向生产核心表决定业务语义。

**理由**：即使源和目标都是 MySQL，旧系统与 PDP 仍存在无符号整数、`TINYINT(1)`、零日期、字符集/排序规则、时区、`ENUM/SET`、JSON、BLOB、自增主键、存储过程和业务语义差异。可靠搬运数据并不等于业务迁移正确。暂存层能保留原始证据、隔离问题并支持幂等重跑；全量加 CDC 可以缩短最终冻结窗口；业务核对用于证明项目层级、审批链、状态和附件未被破坏。
...
## 决策 17：安全、状态机和服务等级属于实现前设计输入

**决定**：威胁模型、数据分类、核心状态机矩阵、高风险操作目录和关键能力服务等级档案在任何业务
故事实现前冻结首版。威胁模型按资产、信任边界、攻击场景、控制措施和残余风险组织；状态机按
前置条件、权限、并发、结果和失败原因组织；服务等级档案按请求分类、交互时限、SLI/SLO、容量、
失败模式、告警、责任人和运行手册组织。

**理由**：如果这些内容延后到发布阶段，权限、数据保护、失败恢复和容量问题会被固化到模型和契约中，
无法满足宪章 DoR，也会让“实现完成”与“可上线”长期脱节。

**备选方案**：

- 发布前统一补做：无法影响已经完成的对象模型、契约和事务边界。
- 只在代码评审中口头检查：缺少版本、责任人和可追踪证据。

## 决策 18：无障碍降为 P3 可选，高风险操作保护保留 P1

**决定**：全部无障碍能力归入 P3 最低优先级可选范围，仅在独立子规格获批后实施，不作为 P1/P2
的实现、验收或发布门禁。领域发布迁移、项目回退、基线替换、人工进度、交付件发布、审批终态、
导出处置和历史迁移上线切换继续统一使用影响预览、明确确认和补偿框架，并保持 P1 强制门禁。

**理由**：当前产品优先保障项目交付闭环、迁移、数据库兼容、安全和高可用，有限资源不在 P1/P2
承担无障碍专项建设；高风险操作若各自实现，仍会出现不可解释、不可撤销和审计口径不一致。

**备选方案**：

- 保留 P1 基础无障碍门禁：与当前最低优先级和可选范围决策冲突。
- 每个模块自行设计确认弹窗：无法统一版本校验、不可逆点和补偿证据。

## 决策 19：契约覆盖以 P1 故事矩阵为准

**决定**：`contracts/coverage.md` 维护 P1 用户故事、HTTP、事件、领域包 Schema、消费者、兼容规则和
验证任务的唯一覆盖矩阵。契约文件在故事实现前达到完整边界，任务顺序固定为“更新契约和失败测试
→ 领域模型与持久化 → 应用服务 → Controller/消费者 → 界面与证据”。

**理由**：仅保留“代表性接口”会允许实现超出契约，也无法识别消费者和弃用影响。

**备选方案**：

- 实现后由路由生成 OpenAPI：不能验证设计兼容性，也违反契约先行。
- 只对外部系统接口建契约：Web 前端和领域扩展同样依赖稳定边界。

## 研究结论

所有技术背景中的不确定项已转化为明确决策。Vue 作为前端基线保持不变；P1 数据库只认证
MySQL 8.4，同时通过持久化端口、逻辑类型和架构测试保持数据库独立；Flowable 8.0.x 作为平台工作流基础能力的受限 BPMN Process Engine 基线，
不改变各领域状态机和审批聚合的事实权威。安全、状态机、服务等级、契约覆盖和高风险操作保护均已前移为实现输入；全部
无障碍能力降为 P3 最低优先级可选范围。P1 实现 `MYSQL→MYSQL` 统一数据库切换；PostgreSQL 适配器、认证及 PostgreSQL/MySQL 产品组合进入 P2；后续新增数据库、改变认证版本、引入跨数据库分片、启用
CMMN/DMN/Flowable REST/IDM/JPA 或开展无障碍建设，必须通过子规格或架构决策记录和完整验证，
不应静默扩大本 L0 计划。
