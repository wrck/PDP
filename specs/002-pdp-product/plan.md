...
- 每个 P1 持久化故事必须同时交付业务模块仓储端口、公共 Liquibase 变更集、公共 Mapper XML 或适配器、必要的数据库专用索引以及 PostgreSQL/MySQL 仓储契约测试；不能只创建领域模型和应用服务。
...
- 建立 MyBatis-Plus 基础设施层、dynamic-datasource 严格路由、每数据源独立 HikariCP、公共持久化端口、PostgreSQL/MySQL 公共适配器和 Liquibase 根变更集。
...
- 建立 CI 质量门禁、容器化本地环境、备份恢复、权限撤销 SLA 和性能基线。
...
- 每个业务增量同步交付公共 DDL、仓储端口、Mapper/适配器和 PostgreSQL/MySQL 契约测试。
...
**结构决策**：后端先保持单一可部署应用，但按业务能力建立强模块边界；后台任务以同一制品的独立运行配置部署。业务模块在自身包内定义领域模型和持久化端口，MyBatis-Plus、Mapper XML、结果映射和数据库实现集中在 `persistence`；每个业务故事通过独立 DDL 与 Mapper 文件落地，禁止把框架类型或 PostgreSQL/MySQL 分支散落在领域模块。Vue 前端按业务特性组织，页面组合进入 `widgets`，可复用业务模型进入 `entities`，跨特性组件、Composable 和元数据渲染运行时只能进入 `shared`。根 pnpm workspace 统一管理前端、契约、Playwright 和辅助测试工具。Pinia Store 只保存跨页面客户端状态，不缓存可由 TanStack Vue Query 管理的服务器事实。

## 复杂度与例外跟踪

无宪章例外。模块化单体、每部署单一写入主库、两种认证数据库适配器和标准 HTTP/事件契约是满足当前范围的最小可行复杂度；不实施运行时跨数据库分片、无迁移热切换或双主写入。消息代理、独立搜索集群和微服务拆分均延后到容量数据或组织边界证明必要时。模块化单体、双数据库适配、动态数据源和迁移架构在实现前分别形成 ADR；后续若申请例外，必须补充违反条款、替代方案、补偿控制、责任人、批准人、失效日期、复审日期和退出条件。
