...
- `data-model.md` 维护工作空间、领域包、项目、阶段、任务、里程碑、基线、交付件、审批、风险、问题、
  变更、后台作业和迁移的确定性状态机。应用服务执行状态迁移，工作流和审批仅协调，不保存唯一事实。
...
- 高风险操作统一接入影响预览、明确确认、审计和撤销/回退/补偿框架，覆盖领域发布迁移、项目回退、
  基线替换、人工进度、交付件发布、审批终态、导出处置和数据库切换。
...
- `public-persistence` 维护逻辑类型、Mapper 端口、公共 SQL 片段、结果映射、查询契约和数据库能力模型；`persistence-postgresql` 与 `persistence-mysql` 只实现差异化 DDL、索引、查询和运维检查。
...
- 在任何业务故事实现前完成模块化单体、双数据库适配、动态数据源和迁移架构四项 ADR，并记录替代
...
| 迁移与数据库切换 | RTO ≤ 30 分钟、RPO ≤ 5 分钟，单写主权 100% | 两次全量彩排和双向认证数据库切换 | 增量落后、核对失败、双主、回退点越界；立即阻断切换 | `datamigration`、`persistence`、DBA 和业务签字人共同负责 |
...
- P1 四条核心旅程必须支持键盘、清晰焦点、可识别名称、非颜色状态和可理解错误；使用自动化扫描、
  组件测试、端到端键盘测试和代表性用户研究联合验收。
- P3 仅包含国际化、屏幕阅读器深度优化和更广泛的无障碍能力，不得把 P1 核心可访问性延后。
...
- 完成四项架构 ADR、威胁模型、数据分类、状态机矩阵、高风险操作目录和 P1 服务等级档案。
...
- 建立 CI 质量门禁、容器化本地环境、审计防篡改校验、备份恢复、权限撤销 SLA、可访问性和性能基线。
...
- 完成核心旅程键盘端到端与自动化可访问性测试，并由代表性键盘用户复核，高等级问题清零。
