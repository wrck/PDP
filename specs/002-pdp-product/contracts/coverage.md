# PDP P1 契约覆盖矩阵

## 用途

本矩阵是 P1 契约完整性的唯一清单。任何 Controller、消费者、领域扩展或 Web 调用在实现前必须
先登记稳定操作、消费者、版本和兼容验证；实际路由与本矩阵或契约文件不一致时发布门禁失败。

| 故事/门禁 | HTTP 契约 | 事件或 Schema | 主要消费者 | 兼容与验证 |
|---|---|---|---|---|
| US1 工作空间治理 | `/workspaces`、状态迁移（激活/暂停/归档/恢复）、负责人转移、`/organizations`（含移动）、`/members`（含暂停/恢复）、`/roles`（含停用）、`/data-scopes`、协作授权及撤销 | 工作空间、组织、成员、角色、数据范围与授权事件 | Web、权限、审计、搜索 | US1 契约测试、撤权与隔离测试 |
| US2 领域包定制 | `/domain-packages`（含详情、PATCH）、`/domain-packages/{id}/versions`（含详情、PATCH 草稿）、`validate`、`validation-result`、`submit`、`reject`、`publish`、`freeze`、`rollback`、`deprecate`、`retire`、`impact-preview`、`diff`、`conflicts`、`snapshot`、`migration-plans`（含 execute、rollback）、`workflow-bindings`、`core-field-catalog` | `domain-package.schema.json`、领域包草稿/校验/审核/发布/冻结/回滚/弃用/退役/迁移事件 | 设计器、运行时、迁移器、`workflow`、审计、搜索 | Schema 校验、状态机测试、三层继承、核心字段复用、冲突检测、影响预览、版本快照、迁移与回滚测试 |
| US3 模板与创建 | `/project-templates`、`/projects` | 项目创建事件 | Web、项目、计划、通知 | 模板版本、幂等和快照测试 |
| US4 项目生命周期 | 项目详情与 `/projects/{id}/transitions` | 项目状态事件 | Web、计划、交付、审计 | 状态机、版本冲突和关闭门禁 |
| US5 任务协作 | 任务创建、转换和协作接口 | 任务状态事件 | Web、进度、通知、搜索 | 状态机、并发、权限和事件兼容 |
| US6 计划基线 | 基线创建、比较和影响预览 | 基线批准/取代事件 | Web、项目、审批 | 循环、权重、版本和差异测试 |
| US7 交付件 | 交付件及版本接口 | 交付件发布事件 | Web、审批、归档、通知 | 内容哈希、签核、不可覆盖和权限 |
| US8 审批 | 审批发起与动作接口 | 审批终态事件、BPMN 流程部署与关联契约 | Web、业务模块、`workflow`、通知 | 节点、轮次、流程版本、撤权、幂等、引擎恢复和回写 |
| US9 多视图 | 项目查询和保存视图接口 | 无新增权威状态事件 | Web | 权限、排序和跨视图一致性测试 |
| US10 风险问题变更 | 风险、问题和变更接口 | 风险/问题/变更状态事件 | Web、审批、计划、通知 | 状态机、差异、补偿和原因测试 |
| US11 权限数据 | 策略、审计、导出、保留和处置接口 | 权限与处置事件 | 管理端、全部业务模块 | 默认拒绝、再鉴权、防篡改和法律保留 |
| US14 搜索通知 | `/search`、`/notifications` | 核心业务事件目录 | Web、投影器、通知器 | 幂等、撤权时限、稳定排序 |
| US18 连续性 | 健康、恢复、服务等级和可用性接口 | 告警和恢复事件 | 运维控制台、值守系统 | SLI/SLO、故障注入和恢复演练 |
| US20 历史迁移 | `/data-migrations`、核对和切换决策 | `migration-report.schema.json`、迁移事件 | 迁移控制台、审计、值守 | 重跑、核对、回退和来源追踪 |
| US21 多数据库 | 当前部署、认证和数据库切换接口 | `migration-report.schema.json`、切换事件 | 运维控制台、迁移器 | 当前数据库语义、单写和双向切换 |
| 横向高风险操作 | `/operation-previews` 及对应确认命令 | 审计事件 | 所有高风险页面和服务 | 预览过期、版本冲突、确认和补偿测试 |
| 横向工作流编排 | `/workflow-definitions/validate`、`/workflow-definitions/deploy`、`/workflow-instances/{id}` 及受控管理动作；不暴露 Flowable API | `pdp.workflow.orchestration.requested/failed` | `workflow`、审批、项目、领域包、运维 | BPMN 校验、版本固定、幂等关联、权限复核、死信和 MySQL 升级 |

## 变更门禁

1. 先更新本矩阵及对应 OpenAPI、事件或 Schema。
2. 增加能够因契约差异而失败的测试，并记录消费者影响。
3. 完成领域模型、持久化和应用实现。
4. 最后实现 Controller、消费者和界面。
5. CI 比较实现路由、事件样例、Schema 和本矩阵，覆盖率必须为 100%。
