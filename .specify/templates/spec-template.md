# 功能规格：[特性名称]

**特性目录**：`[###-feature-name]`

**创建日期**：[DATE]

**状态**：草稿

**输入**：$ARGUMENTS

## 上位追踪与范围

- **规格层级**：[L1/L2/L3/L4]
- **上位规格**：[路径]
- **引用需求**：[用户故事、FR、SC]
- **本次优先级**：[P1/P2/P3]
- **利益相关者与责任人**：[角色]
- **范围内**：[明确内容]
- **非目标**：[明确不做内容]

## 统一领域语言

### 词汇表

| 规范中文名 | 英文稳定键 | 唯一业务含义 | 标准来源或 PDP 扩展 | 允许别名与禁止混用 |
|---|---|---|---|---|
| [术语] | `[stable_key]` | [唯一业务含义] | [ISO/PMI/OMG/PDP 扩展] | [界面别名；不得与何术语混用] |

标准未覆盖的术语必须标记为 PDP 扩展；偏离行业标准时记录理由、影响和批准人。

### 核心对象、关系与不变量

- **[对象]**：[身份、所有权、关系、生命周期和不可违反规则]

### 状态机

- **[对象状态]**：[允许迁移、前置条件、授权、并发语义和失败结果]

## 用户场景与测试（必需）

### 用户故事 1 - [标题]（优先级：P1）

[用户旅程与业务结果]

**优先级理由**：[理由]

**独立测试**：[独立验证方式]

**验收场景**：

1. **给定** [初始状态]，**当** [动作]，**则** [结果与闭环证据]

### 边界场景

- [失败、并发、权限、恢复、迁移、空状态或外部依赖故障]

## 业务闭环与领域规则

- **参与角色**：[角色及责任]
- **触发与输入**：[触发条件、来源和校验]
- **状态变化**：[状态机及关键不变量]
- **业务规则**：[确定性规则及冲突处理]
- **输出与证据**：[结果、审计、通知或交付件]
- **异常与补偿**：[拒绝、重试、回退、人工补偿和不可逆点]

## 需求（必需）

### 功能需求

- **FR-001**：[可测试且不绑定实现的需求]

### 权限、安全与数据治理

- [默认拒绝、授权范围、隔离、威胁、审计、敏感数据、保留和处置规则]

### 契约、兼容与依赖

- [接口和事件契约、版本、消费者、弃用窗口、依赖故障和降级规则]

### 迁移、恢复与运行

- [数据迁移、回退、SLI/SLO、容量、告警、恢复和运行手册要求]

### 用户体验与可解释性

- [角色旅程、跨视图一致性、失败反馈、预览、撤销/补偿和可用性要求；无障碍仅在上位范围明确批准时作为可选要求]

## 成功标准（必需）

### 可衡量结果

- **SC-001**：[技术无关、可量化且可验证的结果]

## 验收与证据

- **验收责任人**：[角色]
- **验证方式**：[自动化、演练、试点或用户研究]
- **证据位置**：[路径或系统]

## 假设

- [合理默认]

## 依赖

- [依赖、所有者和失败影响]

## 风险

- [风险、影响、缓解方向、责任人和复审时间]
