...
**决定**：P1 每个部署使用 MySQL 8.4 LTS 作为唯一业务事实真源。核心字段使用规范化关系模型；可扩展对象使用逻辑 JSON、通用实例、关系和类型化索引投影表。领域层和应用层只依赖持久化端口，MySQL 差异集中在适配器、方言、映射、查询实现和 Liquibase 专用变更集中。PostgreSQL 适配器、认证矩阵和 PostgreSQL/MySQL 跨库切换进入 P2 独立子规格。

**理由**：P1 需要同时完成平台内核、Flowable、标准实施领域包和现有 MySQL 历史数据迁移。双数据库同步实现会扩大 DDL、SQL、索引、性能、恢复和迁移测试面。MySQL 8.4 LTS 能支撑核心 OLTP，并与现有数据和运维能力最接近。通过公共持久化端口、逻辑类型和架构测试保留数据库独立性，可以先降低首期交付风险，再以 P2 认证矩阵验证 PostgreSQL，而不是在 P1 维护两套未经充分验证的实现。
