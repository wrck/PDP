...
**决定**：首个验证基线采用 `dynamic-datasource-spring-boot4-starter 4.5.0`，设置 `primary=pdpPrimary`、`strict=true`。在线路由键为业务主库 `pdpPrimary` 和可选只读副本 `pdpRead`。迁移控制面注册 `migrationSource` 和 `migrationTarget`，但二者分别使用专用 `SqlSessionFactory`、Mapper 扫描包和本地事务管理器。每个数据源使用独立 HikariCP 池，HikariCP 版本由 Spring Boot BOM 管理。
...
## 决策 18：核心可访问性和高风险操作保护纳入 P1

**决定**：查看任务、更新进度、上传交付件和处理待办必须支持键盘、清晰焦点、可识别名称和非颜色
状态表达；自动化检查和键盘端到端测试作为 P1 门禁。领域发布迁移、项目回退、基线替换、人工进度、
交付件发布、审批终态、导出处置和数据库切换统一使用影响预览、明确确认和补偿框架。

**理由**：核心可访问性是基本可用性而非国际化附属能力；高风险操作若各自实现，容易出现不可解释、
不可撤销和审计口径不一致。

**备选方案**：

- 全部延后到 P3：违反核心旅程可访问性要求。
...
所有技术背景中的不确定项已转化为明确决策。Vue 作为前端基线保持不变；数据库采用 PostgreSQL/MySQL
双认证而非单库选型。安全、状态机、服务等级、契约覆盖、核心可访问性和高风险操作保护均已前移为
实现输入。后续新增数据库、改变认证版本或引入跨数据库分片，必须通过子规格或架构决策记录和完整
兼容性测试，不应静默扩大本 L0 计划。
