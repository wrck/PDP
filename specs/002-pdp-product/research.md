...
**决定**：P1 每个部署使用 MySQL 8.4 LTS 作为唯一业务事实真源。核心字段使用规范化关系模型；可扩展对象使用逻辑 JSON、通用实例、关系和类型化索引投影表。领域层和应用层只依赖持久化端口，MySQL 差异集中在适配器、方言、映射、查询实现和 Liquibase 专用变更集中。P1 同时实现持久化适配器注册、数据库能力画像、部署事实、启动校验、单写主权以及数据库切换高风险操作类型等内部扩展契约，并使用模拟适配器验证边界；PostgreSQL 适配器、认证矩阵、跨库迁移执行器和 PostgreSQL/MySQL 跨库切换进入 P2 独立子规格。
...
运行中直接修改连接字符串仍不能安全切换目标，因为 schema 版本、数据内容和写入主权无法自动一致。P1 只支持历史 MySQL 旧系统到 PDP MySQL 的受控上线切换；PDP 认证数据库之间的切换进入 P2。任一时刻只允许一个写入主库；P1 不支持跨数据库分片、在线双主或无迁移热切换。
...
无障碍能力降为 P3 最低优先级可选范围。PostgreSQL 适配器、认证与跨数据库切换进入 P2；后续新增数据库、改变认证版本、引入跨数据库分片、启用
