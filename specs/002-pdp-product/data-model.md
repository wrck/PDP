...
`DatabaseDeploymentProfile` 记录受管数据库部署的认证事实，并作为数据库适配器注册、能力画像、启动校验和切换预检的稳定载体。P1 使用统一 `DatabaseSwitch` 模型完整实现 MySQL 受管部署之间的切换；P2 通过增加数据库适配器和获批组合复用同一模型。领域包运行实例迁移使用前述 `MigrationJob`；旧系统到 PDP 的 `CUTOVER` 与既有 PDP 部署间的 `DATABASE_SWITCH` 分开建模。
...
字段：`id`、`stable_key`、`database_type`、`database_version`、`jdbc_driver_version`、`schema_version`、`character_set`、`collation`、`timezone`、`transaction_engine`、`isolation_level`、`capabilities`、`connection_secret_ref`、`validation_status`、`write_authority`、`validated_at`、`revision`。

规则：配置只保存非敏感部署事实，连接凭据使用密钥引用；启动和切换预检的实际探测结果必须与认证能力矩阵一致。P1 数据库类型仅允许 `MYSQL`，可登记多个部署，但任一时刻只能一个部署的 `write_authority=PRIMARY`。

### 9.3 `DatabaseSwitchCapability`

字段：`id`、`source_database_type`、`target_database_type`、`source_version_range`、`target_version_range`、`required_capabilities`、`migration_package_version`、`status`、`approved_by`、`approved_at`。

规则：P1 仅存在获批的 `MYSQL→MYSQL` 组合；P2 可增加其他产品组合。未获批组合不得创建切换计划。

### 9.4 `DatabaseSwitch`

字段：`id`、`source_deployment_id`、`target_deployment_id`、`capability_id`、`status`、`write_authority`、`planned_at`、`rollback_deadline`、`revision`、`created_by`、`reason`。

状态：`PLANNED → REHEARSING → READY → FREEZING → SWITCHING → VALIDATING → STABILIZING → COMPLETED`，可进入 `ROLLING_BACK/FAILED/CANCELLED`。

规则：源目标部署不得相同；进入 `READY` 前必须完成能力、版本、schema、位点、容量、全量、增量、核对和回退演练；任一状态只能有一个写入主权，`FROZEN` 期间两端均不得接受业务写入。

### 9.5 `DataSourceRegistration`
